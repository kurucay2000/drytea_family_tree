...
class FamilyTreeVisualization:
    ...
    def _create_graph_visualization(self):
        ...
        parent_edges = [
            (u, v) for (u, v, d) in G.edges(data=True) if d["relationship"] == "parent"
        ]
        ...
        spouse_edges = [
            (u, v) for (u, v, d) in G.edges(data=True) if d["relationship"] == "spouse"
        ]
        nx.draw_networkx_edges(
            G,
            pos,
        ...
        ttk.Button(
            button_frame, text="Regenerate Layout", command=self._regenerate_layout
        ).pack(side=tk.LEFT, padx=5)

        # Save visualization button
        ttk.Button(
            button_frame, text="Save Visualization", command=self._save_visualization
        ).pack(side=tk.LEFT, padx=5)
...
def add_visualization_to_ui(family_tree_ui):
    ...
    ttk.Button(
        family_tree_ui.buttons_frame, text="Visualize Tree", command=open_visualization
    ).pack(side=tk.LEFT, padx=5)
